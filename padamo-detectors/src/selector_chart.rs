...
where
    ...
    F2:'static + Fn(Vec<usize>)->Msg,
{
    pub fn new(detector_plotter:&'a DetectorPlotter<Msg>,detector:&'a DetectorAndMask, pixels:&'a Vec<Vec<usize>>, pixels_show:&'a Vec<bool>, lclick_event:Option<F1>, rclick_event:Option<F2>)->Self{
        Self { detector_plotter, detector ,pixels,pixels_show, lclick_event, rclick_event}
    }
}
    ...
    F2:'static + Fn(Vec<usize>)->Msg,
{
    ...
    fn draw_chart<DB: DrawingBackend>(&self, _state: &Self::State, root: DrawingArea<DB, plotters::coord::Shift>) {
        self.detector_plotter.build_chart_aux(&self.detector,&root, self.pixels, self.pixels_show, Margins { top: 5, bottom: 5, left: 5, right: 5 });
