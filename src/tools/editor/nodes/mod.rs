...
pub struct GraphNodeCloneBufferSerializable{
    pub storage:serialization::SerializedNodes,
    pub offset:serialization::SerdePoint,
    pub connections:HashMap<usize,HashMap<String,(usize,String)>>,
}

impl From<GraphNodeCloneBuffer> for GraphNodeCloneBufferSerializable{
    fn from(value: GraphNodeCloneBuffer) -> Self {
        GraphNodeCloneBufferSerializable { storage: value.storage.serialize_to_value(),
...
impl GraphNodeCloneBufferSerializable{
    pub fn instantiate(self, registry: &NodesRegistry)->Result<GraphNodeCloneBuffer,GraphDeserializationError>{
        let mut storage = GraphNodeStorage::new();
        storage.deserialize_from_value(registry, self.storage)?;
        Ok(GraphNodeCloneBuffer {
            storage,
